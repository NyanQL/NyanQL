...
※ 1SQLでも「必ずトランザクションにしたい」場合は、`script` を使うか、SQL を分割して複数指定してください。
...
- **成功**: 全て成功したら `COMMIT`
- **失敗**: 途中でエラーが発生したら `ROLLBACK`（以降の SQL は実行されません）

※ `sql` が **1ファイルのみ**の場合は、通常はトランザクションを開始しません（各 SQL は DB の自動コミット動作になります）。
