SELECT DATE_FORMAT(dates.date, '%Y-%m-%d') AS date,
    DAYOFWEEK(dates.date) AS day_of_week,
    CASE
...
FROM
    (
        SELECT DATE_ADD(DATE_FORMAT(CONCAT( /*year*/'2024', '-', LPAD( /*month*/"5" , 2, '0'), '-01'), '%Y-%m-%d'), INTERVAL t1.i + t2.i * 31 DAY) AS date
        ...
        WHERE DATE_ADD(DATE_FORMAT(CONCAT( /*year*/'2024', '-', LPAD(/*month*/"5" , 2, '0'), '-01'), '%Y-%m-%d'), INTERVAL t1.i + t2.i * 31 DAY) <= LAST_DAY(CONCAT( /*year*/'2024' , '-', LPAD(/*month*/"5" , 2, '0'), '-01'))
    ) AS dates
LEFT JOIN
