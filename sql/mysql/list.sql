...
FROM
    (
        SELECT DATE_ADD(DATE_FORMAT(CURRENT_DATE(), '%Y-%m-01'), INTERVAL t1.i + t2.i * 31 DAY) AS date
        ...
        WHERE DATE_ADD(DATE_FORMAT(CURRENT_DATE(), '%Y-%m-01'), INTERVAL t1.i + t2.i * 31 DAY) <= LAST_DAY(CURRENT_DATE())
    ) AS dates
LEFT JOIN
